...
class MainWindow():
    ...
    def __init__(self, main, image_number=1, language="Marathi", width=726, height=1280):
        ...
        self.my_images = ImageTk.PhotoImage(Image.open("Sample_images/Hoaxbust2_%02d.png" % (self.my_image_number) ).resize([self.width, self.height]))
        ...
        self.df = pandas.read_csv("Hoaxbust2.csv")
    ...
    def render(self):
        a = fill_poster("Sample_images/Hoaxbust2_%02d" % self.my_image_number)
        a.convert(self.my_image_number, self.strings, self.placements, self.language, self.fonts)

        self.my_images = ImageTk.PhotoImage(Image.open("Final/Sample_images/Hoaxbust2_%02d_%s.png" % (self.my_image_number, self.language) ).resize([self.width, self.height]))
    ...
    def update_fonts(self):
        # Fonts
        self.fonts = {}
        # Font1 is regular, font2 is bold
        ...
        self.fonts["5"] = ImageFont.truetype(self.config[self.language]["font1"], size=self.config[self.language]["size5"])
        self.fonts["6"] = ImageFont.truetype(self.config[self.language]["font1"], size=self.config[self.language]["size6"])
        self.fonts["7"] = ImageFont.truetype(self.config[self.language]["font1"], size=self.config[self.language]["size7"])
    ...
    def key_pressed(self, event):
        ...
        if event.char == "l":
            print(self.modify_string + len(self.strings))
